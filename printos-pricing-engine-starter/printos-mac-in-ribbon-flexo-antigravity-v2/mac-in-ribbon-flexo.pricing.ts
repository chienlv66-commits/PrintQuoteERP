 ...
 * - Chốt nghiệp vụ v2:
 *   + Riboon: nền bệt giữ nguyên như Excel, không tác động công thức.
 *   + Riboon: phí khác mặc định 30.000, admin được sửa.
 *   + Flexo: phí ra fim mặc định 25.000, admin được sửa.
 *   + Flexo: số mã trên 1 bài in luôn >= 1, mặc định 1 nếu không nhập.
 * - V2: cập nhật theo xác nhận của khách hàng: Ribbon giữ nguyên nền bệt như Excel; phí khác và phí ra phim cho admin sửa; codeCount luôn >= 1.
 */
...
export interface RibbonInput {
  ...
  otherCost?: number;          // J6 "Khác", mặc định 30000, admin được sửa
...
export interface FlexoInput {
  ...
  codeCount: number;           // K3: số mã trên 1 bài in, luôn >= 1
  cutFlag: 0 | 1;              // L3: nguyên cuộn = 0, cắt TP = 1
  solidBackgroundFlag: 0 | 1;  // M3: không nền bệt = 0, có nền bệt = 1
  filmCost?: number;           // I6/I7/I8 Ra fim, mặc định 25000, admin được sửa
...
function normalizeCodeCount(codeCount?: number): number {
...
export function calculateRibbon(input: RibbonInput): QuoteResult {
  ...
  const otherCost = input.otherCost ?? 30000; // J6 - mặc định 30.000, admin được sửa
 ...
 * J = plateFactor*widthCm*lengthMm*10*5*codeCount; đã chốt codeCount luôn >= 1
...
export function calculateFlexo(input: FlexoInput): QuoteResult {
  ...
  // V2 theo xác nhận: codeCount luôn >= 1, nên luôn dùng công thức bài in có số mã.
